...
results['Second Round'] = [sum(df.loc[i, j]*results.loc[j, 'First Round'] for j in secondround[i]) for i in teams ]
...
# Put second round probabilities in
results['Third Round'] = [sum(df.loc[i, j]*results.loc[j, 'Second Round'] for j in thirdround[i]) for i in teams]
